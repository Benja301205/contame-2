  ...
  return (
    <div className="grid gap-4 text-sm sm:grid-cols-2">
      <div>
        ...
        {byType.map((t) => (
          <p key={t.label}>
            {t.label}: ${t.total.toLocaleString("es-AR", { maximumFractionDigits: 0 })}
          </p>
        ))}
      </div>
      <div>
        <p className="mb-1 font-medium">Por categoría de motivo</p>
        {byReason.length === 0 && <p className="text-muted-foreground">Sin datos.</p>}
        {byReason.map((r) => (
          <p key={r.label}>
            {r.label}: ${r.total.toLocaleString("es-AR", { maximumFractionDigits: 0 })}
          </p>
        ))}
      </div>
    </div>
